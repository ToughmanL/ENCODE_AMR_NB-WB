 ...
 *
 * Parameters:
 *    st                I: pointer to state structure指针状态结构
 *    mode              I: Speech Mode
 *    speech            I: Input speech
 *    serial            O: Output octet structure 3GPP or
 *                         ETSI serial stream输出八位字节结构3GPP或ETSI串行流
      ...
      int force_speech, int filenu)
{
   ...
   if (noHoming){
      Speech_Encode_Frame( s->encoderState, mode, speech, prm, &used_mode ,filenu);
   }
   else {
      switch ( mode ) {
         case MR122:
