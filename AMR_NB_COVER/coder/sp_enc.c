...
//FILE *lag_hv_op;
//int lag_s,max_s;
//int js=0,js_s=0,zs_x=0;//js计数2000个lag,js_s是计数整个2000*738个lag
//int t=0,p=0;//m为隐写的个数是嵌入率的20倍
//void cr(int sz[m] );//调用的用于在2000个基音延迟中产生不重复的m个随机数的位置
//void cs(int ste[m] );//调用需要嵌入的秘密信息
//int sz[m];
//int ste[m];
...
static Word32 Pitch_fr( Word32 *T0_prev_subframe, enum Mode mode, Word32 T_op[],
      ...
      , Word16 *resu3, Word32 *ana_index ,int filenu)
{
   Float32 corr_v[40];
   Float32 max;
   Float32 *corr;
   ...
   if ( ( i_subfr == 0 ) || ( i_subfr == L_FRAME_BY2 ) ) {
	...
	/*lag_hv_op=fopen("C:\\Users\\Administrator\\Desktop\\lag_hv_op_oo.txt","a");
   fprintf(lag_hv_op,"%d\n",T_op[frame_offset]);
   fclose(lag_hv_op);*/
   ...
   /*lag_hv_o=fopen("C:\\Users\\Administrator\\Desktop\\lag_yn_oo.txt","a");
   fprintf(lag_hv_o,"%d\n",lag);
   fclose(lag_hv_o);*/
 ...
 //   srand(time(NULL));	
	...
	//{
	//	cs(ste);//调用函数嵌入隐写信息
	//	cr(sz);//调用函数生成0-2000中m个不重复的需要隐写的位置   
	//}
   ...
   {//delta_search == 0可以认为是一三子帧 lag>95
      /*
   ...
   *pit_frac = frac;
   ...
   if(js==2000)
   {
	   LPITCH[100];
	   FPITCH[100];
	   itoa(filenu,FILENUMBER,10);

	   strcpy(LPITCH,"D:\\MyPaper\\Thesecondpaper\\3.12\\fracfeature\\10s\\0\\lag\\");
	   strcat(LPITCH,FILENUMBER);
	   strcat(LPITCH,"lag.txt");
	   Lag=fopen(LPITCH,"a");

	   strcpy(FPITCH,"D:\\MyPaper\\Thesecondpaper\\3.12\\fracfeature\\10s\\0\\lag\\");
	   strcat(FPITCH,FILENUMBER);
	   strcat(FPITCH,"lagfra.txt");
	   LagF=fopen(FPITCH,"a");
  ...
  /*lag_hv_o=fopen("C:\\Users\\Administrator\\Desktop\\lag_o.txt","a");
   fprintf(lag_hv_o,"%d\n",lag);
   fclose(lag_hv_o);*/
   //printf("%d\n",lag);
   /*lag_hv_o=fopen("C:\\Users\\Administrator\\Desktop\\lag_yn_s.txt","a");
   fprintf(lag_hv_o,"%d\n",lag);
   fclose(lag_hv_o);*/
...
//	FILE *ste_10;
//	int i=0,j=0;
//	ste_10=fopen("C:\\Users\\Administrator\\Desktop\\yn_steg.txt","a");
//	for(i=0;i<m;i++)
//	{
//		j=rand()%2;j--;
//		fprintf(ste_10,"%d\n",j);
...
//	FILE *wz;
//	int i=0,j=0,t=0,k=0;
//	int az[2000];//生成m个2000以内需要隐写的数字
//		/*int sz[2000];*/
//		for(i=0;i<2000;i++)az[i]=i+1;	
//		for(i=0;i<m;i++)
//		{
//			j=rand()%(2000-i);
//			sz[i]=az[j];
//			for(k=j;k<1999-i;k++)az[k]=az[k+1];
//			//printf("%d\n",b[i]);
//		}
//		//判断随机生成的m个数是否会重复
//		/*for(i=0;i<2000;i++)
...
//			}
//		}//判断是否排序成功
//		wz=fopen("C:\\Users\\Administrator\\Desktop\\steg_addr.txt","a");
//		for(i=0;i<m;i++)
//		{
//			fprintf(wz,"%d\n",sz[i]);
//		}fclose(wz);
...
static void cod_amr( cod_amrState *st, enum Mode mode, Float32 new_speech[],
      Word16 ana[], enum Mode *used_mode, Float32 synth[],int filenu )
{
   /* LPC coefficients */
   ...
   Float32 xn2[L_SUBFR];   /* Target vector for codebook search码本搜索的目标矢量 */
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      cl_ltp( &st->clLtpSt->pitchSt->T0_prev_subframe, st->tonStabSt->gp, *
            ...
            lsp_flag, xn2, y1, &T0, &T0_frac, &gain_pit, gCoeff, &ana, &gp_limit,filenu
            );
...
void Speech_Encode_Frame( void *st, enum Mode mode, Word16 *new_speech, Word16 *
      prm, enum Mode *used_mode ,int filenu)
{
   ...
   cod_amr( state->cod_amr_state, mode, speech, prm, used_mode, syn ,filenu);
