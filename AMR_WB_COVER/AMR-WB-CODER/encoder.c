...
int main()
{
	...
	for(filenu=0;filenu<8000;filenu++)//混合样本有3367个
	{
	sprintf(Infile,"E:\\Sample_M-AILABS\\All\\sample%04d.pcm",filenu);
	sprintf(Outfile,"D:\\data\\AMR_WB_COVER\\8000\\10s\\0\\cover\\sample%04d.dat",filenu);
	printf("%s\n%s\n",Infile,Outfile);
...
int coder (char *infilename, char *outfilename){
