...
FILE *LSP_file,*ACB_file,*ACB_gain_file,*FCB_file,*FCB_gain_file;
FILE *lag_d;
int js=0;
int steBit;
/*
...
void Speech_Decode_Frame( void *st, enum Mode mode, Word16 *parm, enum
	  ...
	  char(* outfilepath)[200],int framenu)
{
   ...
   Word32 i,j;
	...
	/*printf("%s\n%s\n%s\n%s\n%s\n",outfilepath[0],outfilepath[1],
		outfilepath[2],outfilepath[3],outfilepath[4]);*/
	if (framenu==1)
	{
		//若已存在文件则先删除再建立
		remove(outfilepath[0]);remove(outfilepath[1]);remove(outfilepath[2]);
		remove(outfilepath[3]);remove(outfilepath[4]);
		if(((LSP_file=fopen(outfilepath[0],"a"))==NULL)||
			((ACB_file=fopen(outfilepath[1],"a"))==NULL)||
			((ACB_gain_file=fopen(outfilepath[2],"a"))==NULL)||
			((FCB_file=fopen(outfilepath[3],"a"))==NULL)||
			((FCB_gain_file=fopen(outfilepath[4],"a"))==NULL))
			printf("The files can't be open\n");
	}
   ...
   for(i=0;i<4;i++)fprintf(LSP_file,"%d\t",LSP_parameter[i]);
   fprintf(LSP_file,"%d\n",LSP_parameter[4]);
   for(i=0;i<4;i++){
		fprintf(ACB_file,"%d\t",ACB_integer[i]);
		fprintf(ACB_gain_file,"%d\t",ACB_Gain_index[i]);
		for(j=0;j<9;j++){fprintf(FCB_file,"%d\t",FCB_pulse[i][j]);}
		fprintf(FCB_file,"%d\n",FCB_pulse[i][9]);
		fprintf(FCB_gain_file,"%d\t",FCB_Gain_index[i]);}
   for(i=0;i<3;i++){
		fprintf(ACB_file,"%d\t",ACB_fraction[i]);
		fprintf(ACB_gain_file,"%d\t",ACB_Gain[i]);
		fprintf(FCB_gain_file,"%d\t",FCB_Gain[i]);
   } 
	fprintf(ACB_file,"%d\n",ACB_fraction[3]);
	fprintf(ACB_gain_file,"%d\n",ACB_Gain[3]);
	fprintf(FCB_gain_file,"%d\n",FCB_Gain[3]);

   if(framenu==500)
   {
		fclose(LSP_file);
		fclose(ACB_file);
		fclose(ACB_gain_file);
		fclose(FCB_file);
		fclose(FCB_gain_file);
   }
